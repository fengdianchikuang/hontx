    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['policy:insured:edit']"
        >修改</el-button>
      </el-col>
<!--      <el-col :span="1.5">-->
<!--        <el-button-->
...
<!--          :disabled="multiple"-->
<!--          @click="handleDelete"-->
<!--          v-hasPermi="['policy:insured:remove']"-->
<!--        >删除</el-button>-->
<!--      </el-col>-->
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['policy:insured:export']"
        >导出</el-button>
      </el-col>
...
<!--      <el-table-column label="主键Id" align="center" prop="qId" />-->
      ...
      <el-table-column label="上传保单" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button
            size="mini"
            type="text"
            icon="el-icon-edit"
            @click="selectUpload(scope.row)"
            v-hasPermi="['policy:insured:edit']"
          >上传</el-button>
        </template>
      </el-table-column>
...
<!--          >删除</el-button>-->
    ...
    <el-dialog :title="title" :visible.sync="open" width="800px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="90px">
        ...
        <el-form-item label="投保人证件类型" prop="applicantCertType">
          <el-select v-model="form.applicantCertType" placeholder="请选择投保人证件类型">
            <el-option v-for="item in insuredCertTypeOption"
                       :key="item.key"
                       :label="item.value"
                       :value="item.key"></el-option>
    ...
    <!-- 上传保单对话框 -->
    <el-dialog :title="title" :visible.sync="openFile" width="500px" @close="cancel" append-to-body>
      <el-form label-width="90px">
        <el-form-item label="交强险保单">
          ...
          >
            ...
            <el-button size="small" type="primary" @click="uploadPositive">点击上传</el-button>

            <div slot="tip" class="el-upload__tip">点击文件,可下载文件</div>
...
<script>
import {getFile, listInsured, getInsured, delInsured, addInsured, updateInsured, exportInsured,uploadFile } from "@/api/policy/insured";
...
export default {
  ...
  methods: {
    ...
    uploadPositive(param){
        ...
        let queryId = this.form.queryId;
        console.log("询价Id",queryId)
        let fileFrom = new FormData();
        fileFrom.append("file",param.file);
        fileFrom.append("queryId",queryId);
        fileFrom.append("type",param.data.type);
        uploadFile(fileFrom).then(res => {
            debugger
            if(res.code === 200 ){
                let obj = new Object();
                obj.requestId = res.data.requestId;
                obj.name = res.data.fileName;
                obj.url = 'http://ecl.oss-cn-hangzhou.aliyuncs.com/' + res.data.path;
                if(res.data.type === '00'){
    ...
    selectUpload(row){
      ...
      this.title = "保单上传";
      console.log("开始上传查询",row.queryId)
        debugger
      getFile(queryId).then(res => {
          console.log("查询上传返回数据",res)
        res.data.forEach(e => {
            console.log("返回单个文件",e)
            if(e.path.indexOf('/home/') != -1){
                console.log("查询")
                let url = 'http://api.echeli.com:8080/upload' + e.path + e.fileName;
                let obj = new Object();
                obj.url = url;
                obj.name = e.fileName;
                obj.queryId = e.queryId;
                obj.fileType = e.fileType;
                obj.id = e.uId;
                if(e.fileType === '00'){
                    this.fileList1.push(obj);
                }else {
                ...
                debugger
                let url = 'http://ecl.oss-cn-hangzhou.aliyuncs.com/' + e.path;
                let obj = new Object();
                obj.url = url;
                obj.name = e.fileName;
                obj.id = e.uId;
    ...
    handleUpdate(row) {
      ...
      // getInsured(qId).then(response => {
      //   this.form = response.data;
      //   this.open = true;
      //   this.title = "修改已投保";
      // });

      this.$router.push({ name: 'insuredEdit', params: { queryId: row.queryId }})
    },
    /** 提交按钮 */
    submitForm() {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除已投保编号为"' + qIds + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delInsured(qIds);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有已投保数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return exportInsured(queryParams);
        }).then(response => {
