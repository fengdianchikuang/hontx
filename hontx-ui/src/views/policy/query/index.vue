    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['policy:query:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['policy:query:export']"
        >导出</el-button>
      </el-col>
...
<!--      <el-table-column label="主键Id" align="center" prop="qId" />-->
      <el-table-column label="询价Id" align="center" prop="queryId" width="200" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="保险Id" prop="channelId">
...
<script>
import { listQuery, getQuery, delQuery, addQuery,getRegionList, updateQuery, exportQuery } from "@/api/policy/query";
...
export default {
  ...
  methods: {
     ...
     * */
    handleEdit(row){
        this.$router.push({ name: 'queryEdit', params: { queryId: row.queryId }})
     },
    /**查询地区表
     * */
    initRegion(){
    ...
    //跳转到详情页
    handleSelect(row){
        this.$router.push({ name: 'queryDetail', params: { queryId: row.queryId }})
    },
    /** 重置按钮操作 */
    resetQuery() {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除已询价列表编号为"' + qIds + '"的数据项?', "警告", {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有已询价列表数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
